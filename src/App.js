...
const formatDate = (date) => new Date(date).toLocaleString('en-GB', { weekday: 'long', day: 'numeric', month: 'long' });
...
const FoodPlanTable = () => {
  ...
  useEffect(() => {
    axios.get(apiUrl + 'week', { params: { date: ISODate(weekStart) }})
      .then(( { data } ) => {
  ...
  useEffect(() => {
    axios.get(apiUrl + 'notes', { params: { date: ISODate(weekStart) }})
  ...
  const handleUpdate = async (e, rowData, key) => {
    ...
    return await axios.post(apiUrl + 'day', { date: rowData.date, [key]: newValue })
      .then(response => {
        if (response.data.message) {
          const newData = data.map(o => o.date === rowData.date ? { ...o, [key]: newValue } : o)
          setData(newData);
          return true;
        }
        return false;
  ...
  const handleNotesUpdate = async (newValue) => {
    ...
    return await axios.post(apiUrl + 'notes', { date: ISODate(weekStart), content: newValue })
      .then(response => {
        if (response.data.message) {
          setNotes(newValue);
          return true;
  ...
  const navigateWeek = (offset) => {
    const newWeekStart = new Date(weekStart.setDate(weekStart.getDate() + (offset * 7)));
  ...
  return (
    <>
      <TableContainer component={Paper} sx={{ overflowY: 'auto' }}>
        <Table sx={{ minWidth: 800, tableLayout: 'fixed' }} aria-label="simple table">
          <colgroup>
            <col />
          ...
          <TableHead>
            <TableRow>
              ...
              <TableCell sx={{ backgroundColor: pink[100], fontWeight: 700 }}>🍳 Breakfast</TableCell>
              <TableCell sx={{ backgroundColor: purple[100], fontWeight: 700 }}>🥗 Lunch</TableCell>
              <TableCell sx={{ backgroundColor: indigo[100], fontWeight: 700 }}>🍝 Dinner</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {data.map((row) => (
              ...
              >
                ...
                <EditableCell handleUpdate={(e) => handleUpdate(e, row, 'breakfast')} content={row.breakfast} />
                <EditableCell handleUpdate={(e) => handleUpdate(e, row, 'lunch')} content={row.lunch} />
                <EditableCell handleUpdate={(e) => handleUpdate(e, row, 'dinner')} content={row.dinner} />
              </TableRow>
            ))}
          </TableBody>
        </Table>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 2 }}>
        <Button variant="contained" startIcon={<ChevronLeftIcon />} onClick={() => navigateWeek(-1)}>
          Previous week
        </Button>
        <Button variant="contained" endIcon={<ChevronRightIcon />} onClick={() => navigateWeek(1)}>
          Next week
        </Button>
      </Box>
      <Grid container spacing={2}>
        <Grid item xs={12}>
          <Paper sx={{ mt: 2, p: 2 }}>
            <EditableMarkdownField content={notes} handleUpdate={handleNotesUpdate} />
          </Paper>
        </Grid>
    ...
    <BrowserRouter>
      <ThemeProvider theme={theme}>
        <Box sx={{ display: 'flex' }}>
          ...
          <Box
            component="main"
            sx={{ flexGrow: 1, p: 3, width: { sm: `calc(100% - ${drawerWidth}px)` }, minHeight: '100vh', backgroundColor: grey[100], width: '100%' }}
          >
            <Toolbar />
            <Routes>
              <Route path="/" element={<p>Welcome to Haus!</p>} />
